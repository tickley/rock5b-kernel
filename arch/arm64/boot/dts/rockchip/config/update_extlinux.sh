...
if [[ -f "/etc/kernel/cmdline" ]]; then
  ...
  APPEND="$APPEND root=PARTUUID=614E0000-0000"
fi
...
emit_kernel() {
  ...
  if [[ -f "/boot/dtb-$VERSION" ]]; then
    ...
    if [[ ! -d "/boot/dtbs/$VERSION" ]]; then
      mkdir -p /boot/dtbs
      cp -au "/usr/lib/linux-image-$VERSION/rockchip" "/boot/dtbs/$VERSION"
    fi
  ...
  if [[ -f "/boot/config.txt" ]]; then
    ...
    do
      if [[ -f "/boot/dtbs/$VERSION/overlay/$DTBO.dtbo" ]]; then
        DTOVERLAY="$DTOVERLAY /dtbs/$VERSION/overlay/$DTBO.dtbo "
      fi
    done
    if [[ -n "$DTOVERLAY" ]]; then
